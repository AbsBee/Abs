/*ASSIGNMENT 2*/
libname priceq '/folders/myfolders/Avinash';

data priceq.priceq(drop=order_number);
	set priceq.price_quote;
run;

proc ttest data=priceq.priceq;
	paired barry_price*mary_price;
run;

/*ASSIGNMENT 5*/
libname class '/folders/myfolders/class';

data class.pricecap;
	set class.per_capita_income;
run;

PROC ANOVA DATA=class.pricecap;
	TITLE 'ONE WAY ANOVA';
	CLASS REGION;
...
PROC ANOVA DATA=class.hospital;
	TITLE 'ONE WAY ANOVA';
	CLASS location;
	...
	/*AST ASSIGNMENTS*/
	/*ASSIGNMENT 3-Randomized Block ANOVA*/
data class.wash;
...
proc anova data=class.order;
	...
	/*AST ASSIGNMENTS*/
	/*ASSIGNMENT 5-Randomized Block ANOVA*/
	FILENAME REFFILE '/folders/myfolders/class/school.csv';

PROC IMPORT DATAFILE=REFFILE DBMS=CSV OUT=class.school;
	GETNAMES=YES;
RUN;

proc contents data=class.school;
proc print data=class.school;
proc anova data=class.school;
